...
namespace Heroes.Icons.HeroesData.Tests
{
    ...
    public class HeroesDataDirectoryTests
    {
        ...
        public void GetVersionsTest()
        {
            ...
            Assert.AreEqual(76268, first.Build);
            Assert.AreEqual(2, first.Major);
            Assert.AreEqual(48, first.Minor);
            Assert.AreEqual(0, first.Revision);
            Assert.IsTrue(first.IsPtr);

            HeroesDataVersion third = list[2];

            Assert.AreEqual(76003, third.Build);
            Assert.AreEqual(2, third.Major);
            Assert.AreEqual(47, third.Minor);
            Assert.AreEqual(2, third.Revision);
            Assert.IsFalse(third.IsPtr);
        }
